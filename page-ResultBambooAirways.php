<?php
/**
 * Template Name: RS Bamboo Airways
 */
...
//if($_POST["enCode"] && $_SESSION["SSID"][$_POST["enCode"]] && $refer == $domain){
    ...
    $res = getSabreInterFlight('QH', $dep, $des, $depdate, $retdate, $direction, $adult, $child, $infant, $is_inter);
    if($is_inter) {
        ...
        $arr=get_flight_from_ws('QH', $dep, $des, $depdate, $retdate, $direction);
            //print_r($arr);
            //exit ;
            $rs=$arr['data'];
            ...
            if($direction==0){
                ...
                for($i=0;$i<count($deprs);$i++){
                    ...
                    $childprice=get_price_child($baseprice,'bambooairways');
                    $infantprice=get_price_infant($baseprice,'bambooairways');

                    $adulttax=getTaxFee_adult(($baseprice),'bambooairways');
                    ...
                    if($child>0){
                        $childtax=getTaxFee_child($baseprice,'bambooairways');
                        $totalchild=($childtax+get_price_child($baseprice,'bambooairways'))*$child;
                        $kq[0][$crkey]['child']=array(
                            'baseprice'=>get_price_child($baseprice,'bambooairways'),
                            'taxfee'=>$childtax,
                            'total'=>$totalchild,
                        );
                    }
                    if($infant>0){
                        $infanttax=getTaxFee_infant($baseprice,'bambooairways');
                        $totalinfant=($infanttax+get_price_infant($baseprice,'bambooairways'))*$infant;
                        $kq[0][$crkey]['infant']=array(
                            'baseprice'=>get_price_infant($baseprice,'bambooairways'),
                            'taxfee'=>$infanttax,
                            'total'=>$totalinfant,
                        );
                    }
